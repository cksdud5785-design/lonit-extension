...
## CI를 못 쓸 때 — 로컬에서 한 줄 게시 (대안)
GitHub Actions가 막혀 있으면(예: 계정 결제 문제) 로컬에서 직접 게시할 수 있습니다. 시크릿은 파일에 저장하지 말고 env로만:
```powershell
$env:CLIENT_ID='...'; $env:CLIENT_SECRET='...'; $env:REFRESH_TOKEN='...'; $env:EXTENSION_ID='...'; $env:PUBLISHER_ID='...'
npm run release:patch    # 버전업 + dist zip 빌드
npm run publish:cws      # dist 최신 zip 업로드 + 게시(검수 제출)
```
(값 5개는 위 1~5단계에서 발급한 것과 동일. `npm run publish:cws` 는 `dist/lonit-extension-*.zip` 중 최신본을 게시)
