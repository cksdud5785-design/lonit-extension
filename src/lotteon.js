...
// 2026-05-26: 새 탭 가시성 fix — 혜택가 fetch 시 탭이 사용자에게 보이던 문제 해결.
...
// 2026-05-26: DETAIL_CONCURRENCY 3→4 (사용자 신고 "수집 속도 개선 필요").
//   롯데온 API 는 안정적이고 detail endpoint 가 별도 도메인 (pbf.lotteon.com) 이라
//   www.lotteon.com 와 rate limit 분리 → 4 까지 안전. 5+ 는 미검증으로 보수적 4.
const DETAIL_CONCURRENCY = 4; // 동시 상세 조회 수
...
async function fetchDetail(pdId, mallNo = 1) {
  try {
    ...
    // 2026-05-16: descInfo.epnJsn 의 DSCRP 항목에서 description HTML 추출 시도.
    //   fileCrtYn === 'Y' 인 경우만 CDN URL 에서 HTML 페이지 fetch.
    //   CDN: https://contents.lotteon.com/p/dn{dtlFileRteNm}{dtlFileNm}.html
    //   실패해도 collector 전체는 정상 진행 (description='' fallback).
    let descriptionHtml = '';
    try {
      const dsc = (d.descInfo?.epnJsn || []).find(e => e.pdEpnTypCd === 'DSCRP' && e.fileCrtYn === 'Y');
      if (dsc && dsc.dtlFileRteNm && dsc.dtlFileNm) {
        const dscUrl = `https://contents.lotteon.com/p/dn${dsc.dtlFileRteNm}${dsc.dtlFileNm}.html`;
        const res = await fetch(dscUrl, {
          headers: { 'Referer': `https://www.lotteon.com/p/product/${pdId}`, 'User-Agent': navigator.userAgent || 'Mozilla/5.0' },
        });
        if (res.ok) {
          descriptionHtml = await res.text();
        }
      }
    } catch (e) {
      console.warn(`[롯데온] description fetch 실패: ${pdId}`, e?.message);
    }

    // Coupang 2026-08-01 정책 — 상품정보고시에 품번/모델번호가 비었을 때 spdNo/pdId 로 보강.
    if (!specs['품번'] && !specs['모델번호']) {
      const fallbackCode = String(bi.spdNo || pdId || '').trim();
...
function parseItem(searchItem, detail, benefit = null) {
  ...
  return {
    ...
    // 2026-05-16: fetchDetail 이 detailImages 를 만들지만 parseItem 이 누락해서 서버 저장
    //   안 됐던 버그 (DB 65000+ row detail_images=0 / description=empty). description 도
    //   동일하게 propagate. 사용자 신고: "롯데온 수집 상품 쿠팡 업로드시 상세페이지가 사라짐".
