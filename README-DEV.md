...
## 다음 단계(선택) — CI 자동 게시
태그 푸시(`v*`) 시 GitHub Actions 로 CWS 자동 업로드/게시 가능(공식 API **V2**, `chromewebstore.googleapis.com`).
필요 사전작업(직접): Google Cloud 에서 Chrome Web Store API 사용 설정 + OAuth 클라이언트 → `client_id/secret/refresh_token` 발급, GitHub repo 시크릿 등록.
도구: `PlasmoHQ/bpp` 또는 `mnao305/chrome-extension-upload@v6`. (참고: 구 V1 API 는 2026-10-15 종료)
